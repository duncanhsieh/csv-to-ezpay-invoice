...
| 商品數量 | 純數字、最長 5 位 | 錯誤 |
| 商品數量 | 非整數自動捨去、0 或負數退回 1 | 警告 |
| 商品數量 | 金額無法被數量整除時退回數量 1（發票金額不受影響） | 警告 |
...
商品名稱與數量可以「逐筆」由來源欄位提供，或不選欄位而「固定」套用整批設定
（下拉選單會直接顯示 `固定 1`、`固定：教育訓練課程` 等實際會用到的值）；
選了逐筆欄位後，對應的預設值欄位會停用並標示「已選逐筆處理」。

規格要求「商品數量 × 商品單價 = 商品小計」且單價必須是整數。當金額無法被數量整除時
（例如 3 個共 1000 元，單價會是 333.33），會退回以數量 1、單價等於小計開立並提出警告——
**發票金額、銷售額與稅額都不受影響**，只有品項上的數量會與實際購買數量不同。
